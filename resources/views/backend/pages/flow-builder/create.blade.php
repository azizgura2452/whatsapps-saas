...
@section('title')
    {{ __('Create Flow Step') }} - {{ config('app.name') }}
@endsection
...
<div class="p-4 mx-auto max-w-4xl md:p-6">
    ...
    <div class="bg-white dark:bg-gray-800 rounded-lg shadow-sm border border-gray-200 dark:border-gray-700 p-6">
        <form action="{{ route('admin.flow-builder.store') }}" 
              method="POST" 
              x-data="flowStepForm()">
            @csrf
                ...
                <div>
                    ...
                    <select name="step_type" required
                        ...
                        @foreach($stepTypes as $value => $label)
                            <option value="{{ $value }}" {{ old('step_type') == $value ? 'selected' : '' }}>
                                {{ $label }}
                            </option>
                        @endforeach
                    </select>
            ...
            <div class="mb-6">
                <div class="flex justify-between items-center mb-4">
                    <h3 class="text-lg font-medium text-gray-900 dark:text-white">
                        {{ __('Messages') }} <span class="text-red-500">*</span>
                    </h3>
            ...
            <div class="mb-6">
                <div class="flex justify-between items-center mb-4">
                    <h3 class="text-lg font-medium text-gray-900 dark:text-white">{{ __('Triggers') }}</h3>
                            ...
                            <div class="grid grid-cols-1 md:grid-cols-3 gap-4">
                                ...
                                <div>
                                    ...
                                    <input type="text" :name="'triggers[' + index + '][trigger_value]'" required
                                           ...
                                           class="w-full rounded-lg border border-gray-300 dark:border-gray-700 bg-white dark:bg-gray-900 px-4 py-2 text-gray-800 dark:text-white">
                                    <p class="text-xs text-gray-500 mt-1">{{ __('Comma-separated for multiple values') }}</p>
                                </div>
                                ...
                                <div>
                                    ...
                                    <select :name="'triggers[' + index + '][next_step_id]'"
                                            ...
                                            class="w-full rounded-lg border border-gray-300 dark:border-gray-700 bg-white dark:bg-gray-900 px-4 py-2 text-gray-800 dark:text-white">
                                        <option value="">{{ __('None (End Flow)') }}</option>
                                    </select>
                                    <p class="text-xs text-gray-500 mt-1">{{ __('Create other steps first to link them') }}</p>
                                </div>
                            </div>
                        </div>
                    </template>
